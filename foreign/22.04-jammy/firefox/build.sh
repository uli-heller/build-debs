...
fakeroot alien -d "${FIREFOX_DP_TAR_BZ2}" -v --version=${NEW_VERSION} -g

cat >>firefox-${NEW_VERSION}/debian/rules <<EOF
override_dh_strip_nondeterminism:
#
EOF

echo >firefox-${NEW_VERSION}/debian/changelog~ "firefox (${NEW_VERSION}) jammy; urgency=low"
tail -n+2 firefox-${NEW_VERSION}/debian/changelog >>firefox-${NEW_VERSION}/debian/changelog~
mv firefox-${NEW_VERSION}/debian/changelog~ firefox-${NEW_VERSION}/debian/changelog

sed -i -e 's/^Architecture: .*$/Architecture: amd64/' firefox-${NEW_VERSION}/debian/control
(
  rm -rf *.orig
  rm -rf *.orig.tar.gz
  cd "firefox-${NEW_VERSION}"
