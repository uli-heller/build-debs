...
THUNDERBIRD_VERSION="$(curl -s "${BASE_URL}"|grep -o "/pub/thunderbird/releases/[0-9][0-9.]*[^/]*/"|grep -v "/12[579]"|grep -v "/13[0-9]"|grep -v "/140"|grep -v "b[^/]*/$"|xargs -n1 basename|sort -V|tail -1)"
